        ...
        <div className="w-full max-w-[1000px] flex flex-col items-stretch gap-12 px-4 py-8">
          ...
          <div className="grid grid-cols-2 gap-2">
            {filteredItems.map((item) => (
              <div
                className="group relative bg-white/0.5 hover:bg-white/1 shadow-[inset_0_0_0_2px_rgba(255,255,255,0.025)] aspect-4/3 rounded-3xl squircle overflow-hidden transition-all duration-200"
                key={item.name}
              >
                {item.asset && (
                  <>
                    <div className="absolute inset-0 -z-1 scale-100 group-hover:scale-105 transition-transform duration-300 ease-out">
                      {item.type === "Video" ? (
                        <video
                          src={item.asset}
                          autoPlay
                ...
                <div className="absolute left-4 group-hover:left-1 bottom-4 group-hover:bottom-1 w-10 group-hover:w-[calc(100%-8px)] h-10 group-hover:h-16 bg-[#1b1b1b] group-hover:bg-[#0e0e0e] border-[1.5px] border-white/5 rounded-[20px] group-hover:rounded-xl overflow-hidden duration-200 ease-in-out"></div>
                <div className="absolute inset-[auto_26px_26px_26px] flex items-center gap-2">
                  ...
                  <div className="flex-1 flex items-center justify-between gap-2 mask-[linear-gradient(to_right,white_33%,transparent_67%)] mask-no-repeat mask-size-[300%_100%] mask-position-[-800px_0] group-hover:mask-position-[0px_0] duration-300 ease-in-out">
                    <h3 className="whitespace-nowrap">
                      {item.series && (
                        <span className="text-white/60">{item.series} / </span>
                      )}
                      <span className="text-white/90">{item.name}</span>
                    </h3>
                    {/* <p className="text-gray-600">{item.description}</p> */}
                    <small className="opacity-0 group-hover:opacity-50 duration-200 delay-100">
                      {item.year}
                    </small>
                  </div>
                </div>
                {item.path && (
                  <a href={item.path} className="absolute inset-0"></a>
                )}
              </div>
            ))}
          </div>
