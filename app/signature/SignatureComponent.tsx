...
export default function SignatureComponent() {
  ...
  useEffect(() => {
    ...
    const name = root.querySelector("#First-name") as HTMLInputElement | null;
    const sign = root.querySelector(".signature-main") as HTMLDivElement | null;
    const uppercase = root.querySelectorAll(
      ".letter-bank .up",
    ) as NodeListOf<HTMLDivElement>;
    const lowercase = root.querySelectorAll(
      ".letter-bank .lo",
    ) as NodeListOf<HTMLDivElement>;
    const signedBy = root.querySelector(".signed-by") as HTMLDivElement | null;
    const modal = root.querySelector(".modal") as HTMLDivElement | null;

    if (!name || !sign || !signedBy || !modal) {
      return;
    }
    ...
    const resetState = () => {
      ...
      modalEl.classList.toggle("active", Boolean(nameEl.value));
    };

    function draw(key: string, animate: boolean) {
      if (key == " ") {
        const space = document.createElement("div");
        space.style.minWidth = "12px";
        signEl.appendChild(space);
        return;
      }
    ...
    return () => {
      ...
      nameEl.removeEventListener("input", handleInput);
      clearScheduled();
      resetState();
      signedByEl.classList.remove("active");
    };
  }, []);
    ...
    >
      <div className="modal">
        ...
        <div className="signature">
          ...
          <div className="signed-by">Signed by,</div>
