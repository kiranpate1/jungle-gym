...
export default function FlightsComponent() {
  ...
  useEffect(() => {
    ...
    const resetToggleState = () => {
      root.classList.remove("active", "lighter");
    ...
    const handleToggleClick = () => {
      const isOpen = root.classList.toggle("active");
    ...
    trayItems.forEach((item, i) => {
      ...
      item.addEventListener("click", handleTrayItemClick);
      trayItemCleanup.set(item, () => {
        item.removeEventListener("click", handleTrayItemClick);
        item.style.opacity = "1";
      });
    });
    ...
    const title = root.querySelector(".filter-icon") as HTMLElement;
    const overflow = root.querySelector(".overflow") as HTMLElement;
    ...
    const flightsList = root.querySelector(".flights-list") as HTMLElement;
    function renderFlights() {
      const dsfkjl = rootRef.current?.querySelector(
        ".flight",
      ) as HTMLElement | null;
      if (!dsfkjl) {
        return;
